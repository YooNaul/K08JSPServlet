...
$(function(){
	...
	$.ajaxSetup({
		...
		type : "get",//전송방식
		contentType : "text/html;charset:utf-8;",//전송박식에 따른 컨텐츠 타입
		dataType : "json",//콜백데이터의 형식
		success : sucFuncJson,//성공했을때 호출할 콜백메서드
		error : errFunc//실패했을때 호출할 콜백메서드
	});
	/*
	앞의 $.ahaxSetup() 메서드에서 모든 속성에 대해 설정이 완료되었으므로
	아래에서는 추가로 설정할 값이 없다. 이경우 속성을 생략해도 된다.
	*/
...
function sucFuncJson(d) {
	...
	/*
	$.each(배열데이터, function(반복시 index 반환, 반복시 해당 객체 반환1){
	*/
